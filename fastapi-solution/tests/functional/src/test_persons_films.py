...
@pytest.mark.parametrize(
    'person_id, expected_response_body',
    [
        (
        ...
        # Персона есть, но нет фильмов.
        ('18e16f7c-84b9-4b18-b2a2-dc6d65fc4e5a', []),
        # Нет персоны и нет фильмов.
...
async def test_persons_get_by_id_cache(
    ...
    film_id = 'b819ed53-ae49-47fb-b6e0-2cf2b40620e0'

    await es_write_data('movies', [
        {
            'id': film_id, 'title': 'Movie 1', 'imdb_rating': 1.1,
            'actors': [{'id': person_id, 'name': 'Ann'}],
            'writers': [], 'directors': []
        }
    ])
    response = await make_get_request(f'/api/v1/persons/{person_id}/film')
    assert len(response['body']) == 1
    assert response['body'][0]['title'] == 'Movie 1'

    # Запрос возвращает данные из кэша, несмотря на обновление в ES.
    await es_write_data('movies', [
        {
            'id': film_id, 'title': 'Movie 2', 'imdb_rating': 2.2,
            'actors': [{'id': person_id, 'name': 'Ann'}],
            'writers': [], 'directors': []
        }
    ])
    ...
    response = await make_get_request(f'/api/v1/persons/{person_id}/film')
    assert len(response['body']) == 1
    assert response['body'][0]['title'] == 'Movie 2'
